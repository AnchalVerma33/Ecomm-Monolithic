...
class OrderService {
  ...
  async InitiateOrder(data) {
    ...
    try {
      ...
      const productArray = productData.filter((obj) => {
      ...
      const dbProdData = await this.productRepository.ProductList({ productID: productArray.map((obj) => obj.productID) });
      ...
      for (let i = 0; i < productArray.length; i++) {
        ...
        const noUnitsLeft = finalProdData[i].quantity - productArray[i].quantity;
      ...
      const razorpayOrder = await this.razorpay.CreateRazorpayOrder({ amount: totalprice * 100, currency });
  ...
  async CancelOrder(data) {
    let doRefund = false;
    try {
      const { filters: { orderID }, user: { id } } = data;
      ...
      const dbProdData = await this.productRepository.ProductList({ productID: orderItems.map((obj) => obj.productID) });
      ...
      const updatedProduct = await this.productRepository.BulkUpdate(productArray);
      const updatedOrder = await this.orderRepository.Update(orderID, { orderStatus: 'Cancelled' });
      if (doRefund) {
        console.log(`Refund initiated for the order amount : ${order.totalAmount}`);
      }
      return updatedOrder;
    } catch (e) {
      throw new APIError(e, e.statusCode);
  ...
  async CompleteOrder(data) {
    try {
      const { filters: { orderID }, user: { id }, details: { paymentID, transactionHash } } = data;
      const order = await this.orderRepository.GetOne({ orderID, userID: id });
      if (!order) {
        return { message: 'Order Not Found' };
      }
      if (['Failed', 'Cancelled', 'Verified', 'Completed'].includes(order.orderStatus)) {
        return { message: `Order Status : ${order.orderStatus}` };
      }
      console.log(`Please pay the amount ${order.totalAmount}`);

      const updatedOrder = await this.orderRepository.Update(orderID, { orderStatus: 'Completed', paymentID, transactionHash });
      return updatedOrder;
    } catch (e) {
      throw new APIError(e, e.statusCode);
    }
    ...
    try {
      ...
      if (infoLevel == 'max') {
        for (const index in orders) {
          ...
          const orderItems = await this.orderItemRepository.GetMany({ orderID: item.dataValues.orderID });
          orders[index].dataValues.orderItems = orderItems.map((obj) => obj.dataValues);
        }
      }
      return orders;
    } catch (e) {
  ...
  async GetOneOrder(filters, infoLevel) {
    try {
      ...
      if (infoLevel == 'max') {
        const orderItems = await this.orderItemRepository.GetMany({ orderID: orders.orderID });
  ...
  async VerifyOrder(filters) {
    try {
      ...
      const {
        orderID, paymentID, transactionHash, orderStatus,
      } = order;
      if (['Failed', 'Cancelled', 'Initiated', 'Verified'].includes(orderStatus)) {
        return { message: `Order Status : ${orderStatus}` };
      }
      const verified = this.razorpay.VerifyOrder({ order_id: orderID, razorpay_payment_id: paymentID, razorpay_signature: transactionHash });

      if (verified) {
        const updateOrderStatus = await this.orderRepository.Update(orderID, { orderStatus: 'Verified', isTransactionVerified: true });
